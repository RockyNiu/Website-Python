...
def load_post(key = 'top', update = False):
	...
	result = memcache.Client.gets(key)
	...
	if update or (key == 'top' and not result):
		...
		memcache.Client.set(key, (list(cursor), int(round(time.time()))))
	elif not result:
		# select all of our blog entries
		result = Entry.get_by_id(key)
		# run query once, grab the results, and cache them
		memcache.Client.set(key, (result, int(round(time.time()))))
	# return all of the posts
	return memcache.Client.gets(key)
